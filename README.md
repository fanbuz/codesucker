...
- ✅ **提交前风险校验** — 检查有效内容、每页行数、末页 2/3、页眉一致性、首末页边界、`@author`/`Copyright` 署名冲突和 HTML+CSS 占比，给出「通过 / 警告 / 退回风险」三级结论
...
**Q：我的代码会被上传吗？**
不会。扫描、清洗、排版和导出全部在本机完成；v0.1.0 不包含自动更新检查。只有你主动点击 GitHub、许可证等外部链接时，系统浏览器才会访问对应网页，源代码不会随之上传。

## 路线图

- [x] **V1（MVP）**：五段流水线 · 5 步向导 · docx/txt 导出 · 多项风险校验 · 配置持久化 · macOS/Windows 安装包
- [ ] **V2**：Linux 安装包 · macOS 签名与公证 · electron-updater 自动更新 · 多目录导入 · 成立日期输入 · 自定义排除/脱敏规则 · 校验项一键修复 · CLI 版本
