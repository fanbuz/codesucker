...
| 系统 | 架构 | 安装包 |
|---|---|---|
| macOS | Apple Silicon（M 系列芯片） | `CodeSucker-0.5.1-mac-arm64.dmg` |
| macOS | Intel | `CodeSucker-0.5.1-mac-x64.dmg` |
| Windows | x64 | `CodeSucker-0.5.1-win-x64.exe` |
...
- 🔎 **可追踪扫描结果** — UTF-8（含 BOM）、GBK、GB18030、UTF-16LE/BE 源码先识别编码再判断二进制；空文件、二进制、超限文件、读取或解码异常都会显示具体文件、原因和处理建议，不再静默消失
- 🧭 **第三方代码线索** — 完全在本机解析 Node.js、Java/Kotlin、Go、Rust、Python 依赖清单，并结合第三方目录、许可证声明和生成代码标记给出证据、置信度与筛选建议；统计区集中展示待核验数量，只提示，不会未经确认自动排除文件
...
- 📝 **一键导出** — docx（页眉=软件名+版本号、右上角自动页码、宋体 10.5pt 固定行距）+ txt 备查，并附带不含源码正文和绝对路径的第三方代码线索 JSON 摘要
...
以下说明只针对 **CodeSucker 桌面应用自身在用户设备上的运行与处理过程**，不延伸到项目开发、维护或文档编写所使用的外部工具。

- CodeSucker 在执行源码扫描、第三方代码线索分析、文件选择、清洗、分页、校验和导出时，**不调用本地或云端任何大语言模型（LLM）**。
- 上述处理不会把源码、源码片段、项目路径、依赖清单或项目配置发送到任何模型服务；处理结果由内置的确定性本地规则产生。
- CodeSucker 不判断源码是否由 AI 生成，也不对代码来源、权利归属或许可证合规作自动法律结论。
- 启动时的更新检测只查询 GitHub Release 的公开版本元数据，不发送源码、项目路径、配置或用户身份数据；核心处理不依赖该网络请求，检测失败也不影响使用。
...
packages/
  core/     纯 TypeScript 流水线，零 Electron 依赖（未来可复用为 CLI / Web）
            discover → third-party clues → clean → select → render → audit
...
**Q：哪些编码、换行和文件大小可以扫描？**
默认支持 UTF-8、UTF-8 BOM、GBK、GB18030、UTF-16LE 和 UTF-16BE；LF、CRLF、CR 都按相同的行语义统计、清洗和导出。单个源码文件不超过 2 MiB（`size <= 2 MiB`）时进入内容扫描，超过该上限会明确列为“超过大小上限”，并显示实际大小与处理建议。空文件、真实二进制文件、无法识别的编码以及读取/解码失败也会保留在扫描报告中。

扫描汇总中的“候选”指：应用 CodeSucker 扫描排除规则后，默认支持后缀下进入发现与内容扫描管线的文件；为了避免遍历 `node_modules` 等已排除大目录，被规则提前剪枝的文件不会虚构为逐文件候选。界面会单独显示本次应用的排除规则数量，`.gitignore` 命中的已发现文件则会以明确的排除原因列出。

**Q：第三方代码线索能自动判断代码归属吗？**
不能。它只根据本机依赖清单、目录约定、许可证声明和生成标记提供可解释的筛选线索，不给出著作权或许可证法律结论。普通 `@author` / `Copyright` 声明不会单独触发第三方代码线索；这类声明会保留到清洗后的提交前校验，并只对最终分页涉及的文件与用户填写的著作权人进行一致性核验。依赖清单只证明项目使用某个依赖，SPDX 也可能是项目自己的开源声明；workspace/path 本地依赖不会默认判为第三方。用户最终勾选的文件仍是预览、分页、校验和导出的唯一范围，未处理线索不会阻止导出。支持格式、处理上限和已知盲区见[第三方代码线索说明](docs/third-party-code-risk.md)。
...
- [x] **v0.5.0**：扫描文件状态与跳过原因全链路可见 · UTF-16/CR 换行可靠处理 · 本地第三方代码风险提示与处理摘要
- [x] **v0.5.1**：第三方代码线索入口移入统计区 · 普通署名不再误报第三方线索 · 明确运行时不使用任何 LLM
