...
| 系统 | 架构 | 安装包 |
|---|---|---|
| macOS | Apple Silicon（M 系列芯片） | `CodeSucker-0.5.0-mac-arm64.dmg` |
| macOS | Intel | `CodeSucker-0.5.0-mac-x64.dmg` |
| Windows | x64 | `CodeSucker-0.5.0-win-x64.exe` |
...
- 🧭 **第三方代码风险提示** — 完全在本机解析 Node.js、Java/Kotlin、Go、Rust、Python 依赖清单，并结合第三方目录、许可证/署名注释和生成代码标记给出证据、置信度与筛选建议；只提示，不会未经确认自动排除文件
...
- 📝 **一键导出** — docx（页眉=软件名+版本号、右上角自动页码、宋体 10.5pt 固定行距）+ txt 备查，并附带不含源码正文和绝对路径的第三方风险 JSON 摘要
...
packages/
  core/     纯 TypeScript 流水线，零 Electron 依赖（未来可复用为 CLI / Web）
            discover → third-party risk → clean → select → render → audit
...
**Q：第三方代码风险提示能自动判断代码归属吗？**
不能。它只根据本机依赖清单、目录约定、许可证/署名注释和生成标记提供可解释的筛选线索，不给出著作权或许可证法律结论。依赖清单只证明项目使用某个依赖，SPDX 也可能是项目自己的开源声明；workspace/path 本地依赖不会默认判为第三方。用户最终勾选的文件仍是预览、分页、校验和导出的唯一范围，未处理提示不会阻止导出。支持格式、处理上限和已知盲区见[第三方代码风险提示说明](docs/third-party-code-risk.md)。
