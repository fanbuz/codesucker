...
- 🔎 **可追踪扫描结果** — UTF-8（含 BOM）、GBK、UTF-16LE/BE 源码先识别编码再判断二进制；空文件、二进制、超限文件、读取或解码异常都会显示具体文件、原因和处理建议，不再静默消失
...
**Q：哪些编码、换行和文件大小可以扫描？**
默认支持 UTF-8、UTF-8 BOM、GBK、UTF-16LE 和 UTF-16BE；LF、CRLF、CR 都按相同的行语义统计、清洗和导出。单个源码文件不超过 2 MiB（`size <= 2 MiB`）时进入内容扫描，超过该上限会明确列为“超过大小上限”，并显示实际大小与处理建议。空文件、真实二进制文件、无法识别的编码以及读取/解码失败也会保留在扫描报告中。
