...
`verify` 会依次执行版本一致性校验、lockfile 公共源检查、品牌图标生成资产一致性校验、测试和完整构建。lockfile 中出现非公共依赖下载地址时必须失败，且检查日志不得回显具体地址。
...
源码当前产品版本为 `0.4.2`。正式发布状态以通过校验的 `v0.4.2` tag 和对应 GitHub Release 为准；仅修改源码中的版本字段不代表已经发布。
