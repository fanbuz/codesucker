...
import { withTemporaryDirectory } from './helpers/temporary-directory.ts';
import {
...
await withTemporaryDirectory('codesucker-real-projects-', async (outputRoot) => {
  const gbkProbe = path.join(outputRoot, 'codesucker_gbk_probe.py');
