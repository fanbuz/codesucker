...
const powerShellSpecialVariableAdjacentComments = [
  '$success=$?# @author PS Success Variable Adjacent Hash',
  '$lastToken=$$# @author PS Last Token Variable Adjacent Hash',
  '$previous=$^# @author PS Previous Variable Adjacent Hash',
  '$successBlock=$?<# @author PS Success Variable Adjacent Block #>+1',
  '$lastTokenBlock=$$<# @author PS Last Token Variable Adjacent Block #>+1',
  '$previousBlock=$^<# @author PS Previous Variable Adjacent Block #>+1',
  '$numeric=$1# @author PS Numeric Variable Adjacent Hash',
  '$numericLong=$123<# @author PS Numeric Variable Adjacent Block #>+1',
  '$arabicNumeric=$١# @author PS Arabic Numeric Variable Adjacent Hash',
  '$superscriptNumeric=$²#literal @author Fake Superscript Numeric Variable Hash',
  '$romanNumeric=$Ⅻ#literal @author Fake Roman Numeric Variable Hash',
  '$scoped=$global:name# @author PS Scoped Variable Adjacent Hash',
  '$scopedNumeric=$script:123<# @author PS Scoped Numeric Variable Adjacent Block #>+1',
  '$successMember=$?.Length# @author PS Special Member Adjacent Hash',
  '$successMemberBlock=$?.Length<# @author PS Special Member Adjacent Block #>+1',
  '$specialSuffix=$?foo# @author PS Special Suffix Adjacent Hash',
  '$nameSuffix=$name?part<# @author PS Named Suffix Adjacent Block #>tail',
  'Write-Output pre$?#literal @author Fake Success Generic Hash',
  'Write-Output pre$$#literal @author Fake Last Token Generic Hash',
  'Write-Output pre$^<#literal @author Fake Previous Generic Block#>tail',
].join('\n');
const powerShellSpecialVariableAdjacentCommentsExpected = [
  '$success=$?',
  '$lastToken=$$',
  '$previous=$^',
  '$successBlock=$? +1',
  ...
  '$romanNumeric=$Ⅻ#literal @author Fake Roman Numeric Variable Hash',
  '$scoped=$global:name',
  '$scopedNumeric=$script:123 +1',
  '$successMember=$?.Length',
  '$successMemberBlock=$?.Length +1',
  '$specialSuffix=$?foo',
  '$nameSuffix=$name?part tail',
  'Write-Output pre$?#literal @author Fake Success Generic Hash',
  'Write-Output pre$$#literal @author Fake Last Token Generic Hash',
  'Write-Output pre$^<#literal @author Fake Previous Generic Block#>tail',
];
assert.deepEqual(
  cleanedLines(powerShellSpecialVariableAdjacentComments, 'ps1'),
  powerShellSpecialVariableAdjacentCommentsExpected,
  'PowerShell confirmed RHS 的 special/numeric/scoped/member 与 special/name suffix expression 后无空白 #/<# 必须开启评论；command generic prefix 内同形标记必须保留',
);
assert.deepEqual(attributionSummary(powerShellSpecialVariableAdjacentComments, 'src/special-variable-adjacent.ps1'), [
  ['author', 'PS Success Variable Adjacent Hash', 1],
  ['author', 'PS Last Token Variable Adjacent Hash', 2],
  ['author', 'PS Previous Variable Adjacent Hash', 3],
  ['author', 'PS Success Variable Adjacent Block', 4],
  ['author', 'PS Last Token Variable Adjacent Block', 5],
  ['author', 'PS Previous Variable Adjacent Block', 6],
  ['author', 'PS Numeric Variable Adjacent Hash', 7],
  ['author', 'PS Numeric Variable Adjacent Block', 8],
  ['author', 'PS Arabic Numeric Variable Adjacent Hash', 9],
  ['author', 'PS Scoped Variable Adjacent Hash', 12],
  ['author', 'PS Scoped Numeric Variable Adjacent Block', 13],
  ['author', 'PS Special Member Adjacent Hash', 14],
  ['author', 'PS Special Member Adjacent Block', 15],
  ['author', 'PS Special Suffix Adjacent Hash', 16],
  ['author', 'PS Named Suffix Adjacent Block', 17],
], 'PowerShell confirmed special/numeric/scoped/member/suffix expression 后真实邻接评论署名必须定位，command generic prefix 中伪署名不得误报');

const powerShellSpecialVariableAdjacentInExpandable = [
  '$message = "prefix $(',
  ...powerShellSpecialVariableAdjacentComments.split('\n'),
  ') suffix" # @author PS Special Variable Expandable Outer Tail',
].join('\n');
assert.deepEqual(cleanedLines(powerShellSpecialVariableAdjacentInExpandable, 'ps1'), [
  '$message = "prefix $(',
  ...powerShellSpecialVariableAdjacentCommentsExpected,
  ') suffix"',
], 'PowerShell ordinary expandable $() 内 special variable 邻接评论与 generic prefix 负例必须复用 top-level 语义并恢复 outer string');
assert.deepEqual(attributionSummary(powerShellSpecialVariableAdjacentInExpandable, 'src/special-variable-adjacent-expandable.ps1'), [
  ['author', 'PS Success Variable Adjacent Hash', 2],
  ['author', 'PS Last Token Variable Adjacent Hash', 3],
  ['author', 'PS Previous Variable Adjacent Hash', 4],
  ['author', 'PS Success Variable Adjacent Block', 5],
  ['author', 'PS Last Token Variable Adjacent Block', 6],
  ['author', 'PS Previous Variable Adjacent Block', 7],
  ['author', 'PS Numeric Variable Adjacent Hash', 8],
  ['author', 'PS Numeric Variable Adjacent Block', 9],
  ['author', 'PS Arabic Numeric Variable Adjacent Hash', 10],
  ['author', 'PS Scoped Variable Adjacent Hash', 13],
  ['author', 'PS Scoped Numeric Variable Adjacent Block', 14],
  ['author', 'PS Special Member Adjacent Hash', 15],
  ['author', 'PS Special Member Adjacent Block', 16],
  ['author', 'PS Special Suffix Adjacent Hash', 17],
  ['author', 'PS Named Suffix Adjacent Block', 18],
...
const powerShellIntegerSuffixAdjacentComments = [
  '$unsigned=1u# @author PS Unsigned Integer Suffix Hash',
  '$unsignedLong=1ul<# @author PS Unsigned Long Integer Suffix Block #>+1',
  '$native=1n# @author PS Native Integer Suffix Hash',
  '$upper=1UL# @author PS Upper Integer Suffix Hash',
  '$hex=0x2Au# @author PS Hex Integer Suffix Hash',
  '$binary=0b101N<# @author PS Binary Integer Suffix Block #>+1',
  '$table=@{',
  'one=1u# @author PS Hashtable Integer Suffix Hash',
  'two=0x2Aul<# @author PS Hashtable Hex Suffix Block #>+1',
  '}',
  '$signedByte=1y# remove signed byte suffix',
  '$unsignedByte=1UY# remove unsigned byte suffix',
  '$signedShort=1s# remove signed short suffix',
  '$unsignedShort=1Us# remove unsigned short suffix',
  '$signedLong=1L# remove signed long suffix',
  '$typedMultiplier=1uLkB# remove typed multiplier',
  '$hexShort=0xFFs# remove hex short suffix',
  '$hexUnsigned=0xFFFFFFFFu# remove hex unsigned suffix',
  '$hexMultiplier=0x10Gb# remove hex multiplier',
  '$binaryMultiplier=0b10nPB# remove binary native multiplier',
  '$decimalInteger=100D# remove decimal suffix',
  '$decimalMultiplier=1.2Dmb# remove decimal multiplier',
  '$longMultiplier=1.2Lgb# remove long real multiplier',
  '$exponentMultiplier=1e2tb# remove exponent multiplier',
  '$invalidRepeat=1uu#literal @author Fake Repeated Integer Suffix',
  '$invalidMixed=1un<#literal @author Fake Mixed Integer Suffix#>tail',
  '$invalidName=1xyz#literal @author Fake Named Integer Suffix',
  '$invalidLongUnsigned=1lu#literal @author Fake Long Unsigned Order',
  '$invalidMultiplierOrder=1gbL#literal @author Fake Multiplier Type Order',
  '$invalidRepeatedTypes=1uyul#literal @author Fake Repeated Type Suffixes',
  '$invalidRealUnsigned=1.2u#literal @author Fake Real Unsigned Suffix',
  '$invalidExponentNative=1e2n#literal @author Fake Exponent Native Suffix',
  '$invalidBinaryDecimal=0b10d#literal @author Fake Binary Decimal Suffix',
  '$invalidHexDigits=0xZZ#literal @author Fake Invalid Hex Digits',
  '$invalidUnsignedName=1uyfoo#literal @author Fake Unsigned Named Suffix',
  'Write-Output pre1u#literal @author Fake Command Integer Suffix Hash',
  'Write-Output pre0x2Aul<#literal @author Fake Command Hex Suffix Block#>tail',
].join('\n');
const powerShellIntegerSuffixAdjacentCommentsExpected = [
  '$unsigned=1u',
  '$unsignedLong=1ul +1',
  '$native=1n',
  '$upper=1UL',
  ...
  '$unsignedShort=1Us',
  '$signedLong=1L',
  '$typedMultiplier=1uLkB',
  '$hexShort=0xFFs',
  '$hexUnsigned=0xFFFFFFFFu',
  '$hexMultiplier=0x10Gb',
  '$binaryMultiplier=0b10nPB',
  '$decimalInteger=100D',
  '$decimalMultiplier=1.2Dmb',
  '$longMultiplier=1.2Lgb',
  '$exponentMultiplier=1e2tb',
  '$invalidRepeat=1uu#literal @author Fake Repeated Integer Suffix',
  '$invalidMixed=1un<#literal @author Fake Mixed Integer Suffix#>tail',
  '$invalidName=1xyz#literal @author Fake Named Integer Suffix',
  '$invalidLongUnsigned=1lu#literal @author Fake Long Unsigned Order',
...
assert.deepEqual(
  cleanedLines(powerShellIntegerSuffixAdjacentComments, 'ps1'),
  powerShellIntegerSuffixAdjacentCommentsExpected,
  'PowerShell confirmed RHS/hashtable 的完整 integer/real/type/multiplier suffix grammar（大小写、hex/bin、组合）后无空白 #/<# 必须开启评论；invalid suffix 与 command generic prefix 必须保留',
);
assert.deepEqual(attributionSummary(powerShellIntegerSuffixAdjacentComments, 'src/integer-suffix-adjacent.ps1'), [
  ['author', 'PS Unsigned Integer Suffix Hash', 1],
  ['author', 'PS Unsigned Long Integer Suffix Block', 2],
  ['author', 'PS Native Integer Suffix Hash', 3],
  ['author', 'PS Upper Integer Suffix Hash', 4],
  ['author', 'PS Hex Integer Suffix Hash', 5],
  ['author', 'PS Binary Integer Suffix Block', 6],
  ['author', 'PS Hashtable Integer Suffix Hash', 8],
  ['author', 'PS Hashtable Hex Suffix Block', 9],
], 'PowerShell合法integer suffix后的真实邻接评论署名必须定位，invalid suffix与command generic中的伪署名不得误报');

const powerShellIntegerSuffixAdjacentInExpandable = [
  '$message = "prefix $(',
  ...powerShellIntegerSuffixAdjacentComments.split('\n'),
  ') suffix" # @author PS Integer Suffix Expandable Outer Tail',
].join('\n');
assert.deepEqual(cleanedLines(powerShellIntegerSuffixAdjacentInExpandable, 'ps1'), [
  '$message = "prefix $(',
  ...powerShellIntegerSuffixAdjacentCommentsExpected,
  ') suffix"',
], 'PowerShell ordinary expandable $() 内 integer suffix 邻接评论与 invalid/command负例必须复用top-level语义并恢复outer string');
assert.deepEqual(attributionSummary(powerShellIntegerSuffixAdjacentInExpandable, 'src/integer-suffix-adjacent-expandable.ps1'), [
  ['author', 'PS Unsigned Integer Suffix Hash', 2],
  ['author', 'PS Unsigned Long Integer Suffix Block', 3],
  ['author', 'PS Native Integer Suffix Hash', 4],
  ['author', 'PS Upper Integer Suffix Hash', 5],
  ['author', 'PS Hex Integer Suffix Hash', 6],
  ['author', 'PS Binary Integer Suffix Block', 7],
  ['author', 'PS Hashtable Integer Suffix Hash', 9],
  ['author', 'PS Hashtable Hex Suffix Block', 10],
  ['author', 'PS Integer Suffix Expandable Outer Tail', 39],
], 'PowerShell expandable $()内合法integer suffix评论与outer tail必须定位，invalid/command伪署名不得误报');
...
const powerShellTypeLiteralAdjacentComments = [
  '$simple=[string]# @author PS Simple Type Literal Adjacent Hash',
  '$generic=[System.Collections.Generic.List[string]]<# @author PS Generic Type Literal Adjacent Block #>+1',
  '$nested=[System.Collections.Generic.Dictionary[string,System.Collections.Generic.List[int]]]<# @author PS Nested Type Literal Adjacent Block #>+1',
  '$array=[string[]]<# @author PS Array Type Literal Adjacent Block #>+1',
  'Write-Output pre[string]#literal @author Fake Generic Type Literal Hash',
  '$groupTypeArgument=(Write-Output [string]#literal @author Fake Group Type Argument Hash)',
  '$groupTypeBlock=(Write-Output [string]<#literal @author Fake Group Type Argument Block#>tail)',
  '$emptyType=[]#literal @author Fake Empty Type Literal',
  '$numericType=[1bad]#literal @author Fake Numeric Type Literal',
  '$spacedType=[int foo]#literal @author Fake Spaced Type Literal',
  '$unclosedType=[int#literal @author Fake Unclosed Type Literal',
  '$typeSuffix=[int]suffix#literal @author Fake Type Literal Suffix',
  '$semicolonType=[int;Write-Output]#literal @author Fake Semicolon Type Literal',
  '$pipeType=[int|foo]#literal @author Fake Pipe Type Literal',
  '$ampersandType=[int&foo]#literal @author Fake Ampersand Type Literal',
  '$commaTypeArgument=(Write-Output foo, [string]#literal @author Fake Comma Type Argument Hash)',
  '$commaTypeBlock=(Write-Output foo, [string]<#literal @author Fake Comma Type Argument Block#>tail)',
].join('\n');
const powerShellTypeLiteralAdjacentCommentsExpected = [
  '$simple=[string]',
  '$generic=[System.Collections.Generic.List[string]] +1',
  '$nested=[System.Collections.Generic.Dictionary[string,System.Collections.Generic.List[int]]] +1',
  '$array=[string[]] +1',
  'Write-Output pre[string]#literal @author Fake Generic Type Literal Hash',
  '$groupTypeArgument=(Write-Output [string]#literal @author Fake Group Type Argument Hash)',
  '$groupTypeBlock=(Write-Output [string]<#literal @author Fake Group Type Argument Block#>tail)',
...
assert.deepEqual(
  cleanedLines(powerShellTypeLiteralAdjacentComments, 'ps1'),
  powerShellTypeLiteralAdjacentCommentsExpected,
  'PowerShell confirmed RHS 的 simple/generic/nested/array type literal 后无空白 #/<# 必须开启评论；generic prefix与group command独立type argument必须保留',
);
assert.deepEqual(attributionSummary(powerShellTypeLiteralAdjacentComments, 'src/type-literal-adjacent.ps1'), [
  ['author', 'PS Simple Type Literal Adjacent Hash', 1],
  ['author', 'PS Generic Type Literal Adjacent Block', 2],
  ['author', 'PS Nested Type Literal Adjacent Block', 3],
  ['author', 'PS Array Type Literal Adjacent Block', 4],
], 'PowerShell confirmed type literal 后真实邻接评论署名必须定位，command type argument中的伪署名不得误报');

const powerShellTypeLiteralAdjacentInExpandable = [
  '$message = "prefix $(',
  ...powerShellTypeLiteralAdjacentComments.split('\n'),
  ') suffix" # @author PS Type Literal Expandable Outer Tail',
].join('\n');
assert.deepEqual(cleanedLines(powerShellTypeLiteralAdjacentInExpandable, 'ps1'), [
  '$message = "prefix $(',
  ...powerShellTypeLiteralAdjacentCommentsExpected,
  ') suffix"',
], 'PowerShell ordinary expandable $() 内type literal邻接评论与command type argument负例必须复用top-level语义并恢复outer string');
assert.deepEqual(attributionSummary(powerShellTypeLiteralAdjacentInExpandable, 'src/type-literal-adjacent-expandable.ps1'), [
  ['author', 'PS Simple Type Literal Adjacent Hash', 2],
  ['author', 'PS Generic Type Literal Adjacent Block', 3],
  ['author', 'PS Nested Type Literal Adjacent Block', 4],
  ['author', 'PS Array Type Literal Adjacent Block', 5],
...
const batchCaretPhysicalLineContinuation = [
  'echo hello ^',
  'REM literal @author Fake Continued Rem',
  'REM @author Batch After Continued Rem',
  'echo triple ^^^',
  '@REM literal @author Fake Triple Continued Rem',
  '@REM @author Batch After Triple Continued Rem',
  'echo even ^^',
  'REM @author Batch Even Caret Real Rem',
  'echo label ^',
  ':: literal @author Fake Continued Label',
  'REM @author Batch After Continued Label',
  'echo carry ampersand ^',
  '& REM literal @author Fake Carry Ampersand Rem',
  'REM @author Batch After Carry Ampersand',
  'echo carry caret ^',
  '^',
  'REM @author Batch After Carry Caret',
  'echo carry quote ^',
  '" & REM @author Batch Carry Quote Separator',
  'echo chain ^',
  'x & echo next ^',
  'x & REM @author Batch Continued Chain Rem',
  'echo "quoted ^',
  'REM @author Batch Quote Caret No Continuation',
].join('\n');
assert.deepEqual(cleanedLines(batchCaretPhysicalLineContinuation, 'bat'), [
  'echo hello ^',
  'REM literal @author Fake Continued Rem',
  'echo triple ^^^',
  '@REM literal @author Fake Triple Continued Rem',
  ...
  'x',
  'echo "quoted ^',
], 'Batch 行末 odd/three caret 必须仅把下一物理行首字符视为 literal；REM/@REM/::、leading &/quote/caret 的 carry-in 与后续真实 separator，even caret、quote内caret及多行chain边界必须正确恢复');
assert.deepEqual(
  extractAttributions(batchCaretPhysicalLineContinuation, 'scripts/caret-physical-continuation.bat', 'bat')
    .map((item) => [item.kind, item.subject, item.line]),
  [
    ['author', 'Batch After Continued Rem', 3],
    ['author', 'Batch After Triple Continued Rem', 6],
    ['author', 'Batch Even Caret Real Rem', 8],
    ['author', 'Batch After Continued Label', 11],
    ['author', 'Batch After Carry Ampersand', 14],
    ['author', 'Batch After Carry Caret', 17],
    ['author', 'Batch Carry Quote Separator', 19],
    ['author', 'Batch Continued Chain Rem', 22],
    ['author', 'Batch Quote Caret No Continuation', 24],
  ],
  'Batch carry-in REM/@REM/::/leading & 行内伪署名不得误报，carry消费后、even caret、quote/caret首字符、multi-line chain 与 quote caret 后真实评论署名必须恢复定位',
);
...
const batchDirectIfRemComments = [
  'if exist foo REM @author Batch If Exist Rem',
  'IF NOT EXIST foo @ReM @author Batch If Not Exist Rem',
  'if errorlevel 1 rem @author Batch If Errorlevel Rem',
  'if defined READY REM @author Batch If Defined Rem',
  'if /I "ready"=="READY" REM @author Batch If String Compare Rem',
  'if not errorlevel 1 REM @author Batch If Not Errorlevel Rem',
  'if cmdextversion 2 REM @author Batch If Cmdextversion Rem',
  'if not cmdextversion 2 @REM @author Batch If Not Cmdextversion Rem',
  'if not defined READY REM @author Batch If Not Defined Rem',
  'if 2 EQU 2 REM @author Batch If Equ Rem',
  'if 2 NEQ 3 REM @author Batch If Neq Rem',
  'if 1 LSS 2 REM @author Batch If Lss Rem',
  'if 1 LEQ 1 REM @author Batch If Leq Rem',
  'if 3 GTR 2 REM @author Batch If Gtr Rem',
  'if 3 GEQ 3 REM @author Batch If Geq Rem',
  'if exist foo^ bar REM @author Batch If Escaped Space Operand Rem',
  'if exist "foo|bar" REM @author Batch If Quoted Pipe Operand Rem',
  'echo before & if exist foo REM @author Batch Chained If Rem',
  'if exist foo|bar REM @author Fake Raw Pipe Operand Rem',
  'if exist foo&bar REM @author Fake Raw Ampersand Operand Rem',
  'if exist foo(bar REM @author Fake Raw Open Paren Operand Rem',
  'if exist foo)bar REM @author Fake Raw Close Paren Operand Rem',
  'if exist foo<bar REM @author Fake Raw Less Operand Rem',
  'if exist foo>bar REM @author Fake Raw Greater Operand Rem',
  'if exist foo^ REM @author Fake Escaped Space Consumes Rem Operand',
  'if exist "unterminated REM @author Fake Unterminated Quoted Operand',
  'if defined foo|bar REM @author Fake Defined Raw Pipe Name',
  'if defined foo&bar REM @author Fake Defined Raw Ampersand Name',
  'if defined foo(bar REM @author Fake Defined Raw Paren Name',
  'if defined foo>bar REM @author Fake Defined Redirect Name',
  'if defined foo^ REM @author Fake Defined Terminal Caret Name',
  'if defined "unterminated REM @author Fake Defined Unterminated Quote Name',
  'echo if exist foo REM @author Fake Echo If Text',
  '"if exist foo REM @author Fake Quoted If Text"',
  '^if exist foo REM @author Fake Escaped If Text',
  'if exist foo echo REM @author Fake If Echo Command',
  'echo before | if exist foo REM @author Fake Single Pipe If Rem',
  'if exist REM file @author Fake If Operand Named Rem',
].join('\n');
assert.deepEqual(cleanedLines(batchDirectIfRemComments, 'cmd'), [
  'echo before',
  'if exist foo|bar REM @author Fake Raw Pipe Operand Rem',
  'if exist foo&bar REM @author Fake Raw Ampersand Operand Rem',
  'if exist foo(bar REM @author Fake Raw Open Paren Operand Rem',
  ...
  'echo before | if exist foo REM @author Fake Single Pipe If Rem',
  'if exist REM file @author Fake If Operand Named Rem',
], 'Batch合法IF EXIST/NOT EXIST/ERRORLEVEL/DEFINED//I compare后的direct REM必须删除整个IF segment；echo/quoted/escaped/echo-command/single-pipe/malformed负例必须保留');
assert.deepEqual(
  extractAttributions(batchDirectIfRemComments, 'scripts/direct-if-rem.cmd', 'cmd')
    .map((item) => [item.kind, item.subject, item.line]),
  [
    ['author', 'Batch If Exist Rem', 1],
    ['author', 'Batch If Not Exist Rem', 2],
    ['author', 'Batch If Errorlevel Rem', 3],
    ['author', 'Batch If Defined Rem', 4],
    ['author', 'Batch If String Compare Rem', 5],
    ['author', 'Batch If Not Errorlevel Rem', 6],
    ['author', 'Batch If Cmdextversion Rem', 7],
    ['author', 'Batch If Not Cmdextversion Rem', 8],
    ['author', 'Batch If Not Defined Rem', 9],
    ['author', 'Batch If Equ Rem', 10],
    ['author', 'Batch If Neq Rem', 11],
    ['author', 'Batch If Lss Rem', 12],
    ['author', 'Batch If Leq Rem', 13],
    ['author', 'Batch If Gtr Rem', 14],
    ['author', 'Batch If Geq Rem', 15],
    ['author', 'Batch If Escaped Space Operand Rem', 16],
    ['author', 'Batch If Quoted Pipe Operand Rem', 17],
    ['author', 'Batch Chained If Rem', 18],
  ],
  'Batch合法direct IF REM真实署名必须定位，各类无法证明语境的伪署名不得误报',
);
