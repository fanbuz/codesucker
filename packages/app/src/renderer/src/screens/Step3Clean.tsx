...
export default function Step3Clean() {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', minHeight: 0, animation: 'cs-fade .18s ease-out' }}>
      <div style={{ width: 420, flex: 'none', borderRight: '1px solid var(--border2)', background: 'var(--panel)', overflow: 'auto', padding: '22px 24px', display: 'flex', flexDirection: 'column', gap: 20 }}>
        <div>
          ...
          {TOGGLES.map((t) => {
            ...
            return (
              <div key={t.key} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', border: '1px solid var(--border2)', borderRadius: 9, background: 'var(--panel2)' }}>
                ...
                <div onClick={() => s.set({ clean: { ...s.clean, [t.key]: !on }, processData: null })}
                  style={{ width: 34, height: 20, flex: 'none', borderRadius: 10, background: on ? 'var(--accent)' : 'var(--border)', position: 'relative', cursor: 'pointer', transition: 'background .15s' }}>
        ...
        <div style={{ border: '1px solid var(--border2)', borderRadius: 9, overflow: 'hidden' }}>
          <div onClick={() => s.set({ layoutOpen: !s.layoutOpen })}
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '11px 12px', cursor: 'pointer', background: 'var(--panel2)' }}>
          ...
          {s.layoutOpen && (
            <div style={{ padding: 12, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, borderTop: '1px solid var(--border2)' }}>
      ...
      {/* 实时预览 */}
      <div style={{ flex: 1, minWidth: 0, background: 'var(--bg)', overflow: 'auto', padding: '20px 24px' }}>
        {!p?.preview ? (
