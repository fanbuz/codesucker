...
await assert.rejects(removeRecentProjects(activeConfig, 'not-an-array'), /Invalid recent project path list/i);
await assert.rejects(removeRecentProject(activeConfig, 'relative/path'), /Invalid recent project path/i);
await assert.rejects(setRecentProjectPinned(activeConfig, project('unknown'), true), /does not exist/i);
